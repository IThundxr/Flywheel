...
//	private static final int WRITE_MASK_LENGTH = Mth.ceillog2(WriteMask.values().length);
	...
	private static final int USE_OIT_LENGTH = 1;
...
//	private static final int WRITE_MASK_OFFSET = TRANSPARENCY_OFFSET + TRANSPARENCY_LENGTH;
	private static final int USE_OVERLAY_OFFSET = BACKFACE_CULLING_OFFSET + BACKFACE_CULLING_LENGTH;
	private static final int USE_OIT_OFFSET = USE_OVERLAY_OFFSET + USE_OVERLAY_LENGTH;
	private static final int USE_LIGHT_OFFSET = USE_OIT_OFFSET + USE_OIT_LENGTH;
...
//	private static final int WRITE_MASK_MASK = bitMask(WRITE_MASK_LENGTH, WRITE_MASK_OFFSET);
	...
	private static final int USE_OIT_MASK = bitMask(USE_OIT_LENGTH, USE_OIT_OFFSET);
	...
	// Packed format:
	// ambientOcclusion[1] | cardinalLightingMode[2] | useLight[1] | useOit[1] | useOverlay[1] | backfaceCulling[1] | mipmap[1] | blur[1]
...
//		bits |= (material.writeMask().ordinal() << WRITE_MASK_OFFSET) & WRITE_MASK_MASK;
		...
		if (material.useOit()) bits |= USE_OIT_MASK;
