package dev.engine_room.flywheel.backend.engine.indirect.deprecated.gl.array;
	...
	public void bindAttributes(final int bindingIndex, final int startAttribIndex, VertexFormat vertexFormat) {
		...
		for (VertexFormatElement element : vertexFormat.getElements()) {
			...
			if (!element.equals(vertexFormatElements[attribIndex])) {
				...
				if (isIntegerFormat) {
					ARBVertexAttribBinding.glVertexAttribIFormat(attribIndex, channelCount, glType, element.offset());
				} else {
					ARBVertexAttribBinding.glVertexAttribFormat(attribIndex, channelCount, glType, isNormalizedFormat, element.offset());
				}
